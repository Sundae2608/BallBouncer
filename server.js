...
function connected(socket) {
  socket.on('newPlayer', data => {
    ...
    serverBalls[socket.id] = new Ball(data.x, data.y, 40, 5);
    serverBalls[socket.id].maxSpeed = 5;
    playerPositions[socket.id] = data;
    console.log("Starting position: " + playerPositions[socket.id].x + " - " + playerPositions[socket.id].y);
    console.log("Current number of players: " + Object.keys(playerPositions).length);
    console.log("Players dictionary: ", playerPositions);
    console.log("---")
...
function serverLoop() {
  ...
  for (let id in serverBalls) {
    playerPositions[id].x = serverBalls[id].pos.x;
    playerPositions[id].y = serverBalls[id].pos.y;
    playerPositions[id].angle = serverBalls[id].angle;
  }
